...
app.config(function ($stateProvider, $urlRouterProvider) {
  $stateProvider
    ...
    /*不能为同一状态配置两个模块，状态名重复了*/
    //.state('order',{url:'/order',templateUrl:'tpl/order.html',controller:'orderCtrl'})
...
//main 的控制器
app.controller('mainCtrl', ['$scope', '$http', '$ionicLoading', function ($scope, $http, $ionicLoading) {
  ...
  $scope.show = function () {/*/显示*/
    $ionicLoading.show({
      template: '正在加载.....'
    });
    setTimeout($scope.hide,2000);
  };

  $scope.hide=function(){/*隐藏*/
    $ionicLoading.hide();
    $scope.loadMore();
  }
